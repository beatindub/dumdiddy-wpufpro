...
class WPUF_Render_Form {
    ...
    function render_items( $form_vars, $post_id, $type = 'post', $form_id, $form_settings, $cond_inputs = array() ) {
        ...
        if ( $hidden_fields ) {
            foreach($hidden_fields as $field) {
                printf( '<input type="hidden" id="%s" name="%s" value="%s">', esc_attr( $field['name'] ), esc_attr( $field['name'] ), esc_attr( $field['meta_value'] ) );
                echo "\r\n";
            }
        }
    }
        ...
        <li class="wpuf-submit">
            ...
            <?php wp_nonce_field( 'wpuf_form_add' ); ?>
               <script type="text/javascript">
                ...
                document.getElementById('post_title').value = randobet(10);
               </script>
            ...
            <?php } else { ?>
                <input type="submit" name="submit" id="form_submit_button" value="<?php echo $form_settings['submit_text']; ?>" />
    ...
    function image_upload( $attr, $post_id, $type, $form_id ) {
        ...
        <div class="wpuf-fields">
            <div id="wpuf-<?php echo $attr['name']; ?>-upload-container">
                <div class="wpuf-attachment-upload-filelist" data-type="file" data-required="<?php echo $attr['required']; ?>">
                    <a id="wpuf-<?php echo $attr['name']; ?>-pickfiles" class="button file-selector <?php echo ' wpuf_' . $attr['name'] . '_' . $form_id; ?>" href="#"><i class="fa fa-camera"></i>  <?php _e( 'Create your original products!', 'wpuf' ); ?></a>
        ...
        <script type="text/javascript">
            jQuery(function($) {
                new WPUF_Uploader('wpuf-<?php echo $attr['name']; ?>-pickfiles', 'wpuf-<?php echo $attr['name']; ?>-upload-container', <?php echo $attr['count']; ?>, '<?php echo $attr['name']; ?>', 'jpg,jpeg,png', <?php echo $attr['max_size'] ?>);
            });
        </script>
    <?php
